...
class BillingViewModel(
    ...
    private fun getTransactions(data: List<BillingDataModel>, uid: String): Transactions {
        val totalPrice = data.sumOf { it.sellingPrice.toDouble() }
    ...
    private suspend fun billItems(
        ...
        val billedItems = serialsToBill.map { serial ->
            SingleItem(
                ...
                sellingPrice = itemDocument.getString("sellingPrice") ?: "",
    ...
    fun generateInvoiceData(
        ...
        val productInformationList = listData.map { billingItem ->
            Invoice.ProductInformation(
                ...
                rate = billingItem.sellingPrice.toDouble(),
                discount = 0.0,
                total = billingItem.sellingPrice.toDouble()
            )
        }
